...
contract DEX {
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		...
		uint256 tokenValueEqualToEth = price(
			msg.value,
			(address(this).balance) - (msg.value),
			token.balanceOf(address(this))
		);
		require(
			token.transferFrom(msg.sender, address(this), tokenValueEqualToEth),
			"token could not receive"
		);
		...
		emit LiquidityProvided(
			...
			msg.value,
			msg.value,
			tokenValueEqualToEth
		);
		return tokenValueEqualToEth;
	}
